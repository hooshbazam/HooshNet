...
class MarzbanPanelManager:
    ...
    def update_client_expiration(self, inbound_id: int, client_uuid: str, expiry_timestamp: int, client_name: str = None) -> bool:
        """
        Update client expiration (for renewal) in Marzban
        ...
        try:
            ...
            # Marzban uses username. Prefer client_name if provided, else client_uuid
            ...
            update_data = {
                "expire": expiry_timestamp, # Marzban uses seconds (Unix timestamp)
                "proxies": current_user.get('proxies', {}),
            ...
            if response.status_code == 200:
                logger.info(f"✅ Successfully updated Marzban user expiration")
                return True
            else:
                logger.error(f"❌ Failed to update Marzban user: {response.status_code}")
                try:
            ...
            logger.error(f"❌ Error updating Marzban user expiration: {e}")
            return False
    ...
    def enable_client(self, inbound_id: int, client_uuid: str, client_name: str = None) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"✅ Successfully enabled Marzban user: {username}")
                return True
            else:
                print(f"❌ Failed to enable Marzban user: {response.status_code}")
                return False
            
        except Exception as e:
            print(f"❌ Error enabling Marzban user: {e}")
            return False
